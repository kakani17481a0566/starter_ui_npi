...
import { collectionList } from "./fakeData";
...
export function FeeStructureTable() {
  ...
  const table = useReactTable({
    ...
    meta: {
      deleteRow: (row) => {
        ...
        setCollection((old) =>
          old.filter(
            (oldRow) => oldRow.collection_id !== row.original.collection_id,
          ),
        );
      },
      deleteRows: (rows) => {
        // Skip page index reset until after next rerender
        skipAutoResetPageIndex();
        const rowIds = rows.map((row) => row.original.collection_id);
        setCollection((old) =>
          old.filter((row) => !rowIds.includes(row.collection_id)),
        );
      },
    },
    filterFns: {
  ...
  useDidUpdate(() => table.resetRowSelection(), [collection.length]);
