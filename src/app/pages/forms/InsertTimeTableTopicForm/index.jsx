...
const InsertTimeTableTopicForm = ({ onSuccess }) => {
  ...
  // Find selected objects (treat IDs as strings!)
  ...
  // Reset lower-level dropdowns if selection above changes
  useEffect(() => {
    setValue("subjectId", "");
  ...
  // Filter periods by date (robust version)
  const filteredTimeTableDetails =
    subjectObj?.timeTableDetails?.filter(ttd => {
      if (!selectedDate || !ttd.date) return false;
      // Only compare the date parts (YYYY-MM-DD)
      return ttd.date.slice(0, 10) === selectedDate;
    }) || [];

  // --- Debugging (uncomment to inspect state) ---
  // console.log("Selected Date:", selectedDate);
  // console.log("Available Dates:", subjectObj?.timeTableDetails?.map(t => t.date));
  // console.log("Filtered Results:", filteredTimeTableDetails);
    ...
    <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
      <Card className="text-primary-950 dark:bg-dark-700 space-y-6 p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          {/* Time Table Detail Dropdown (filtered by selected date) */}
          <Controller
          ...
          {/* Read-only Tenant and Created By */}
          <Input
            label="Tenant ID"
