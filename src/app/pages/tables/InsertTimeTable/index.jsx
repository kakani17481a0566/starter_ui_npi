...
import { generateTimeTableColumns } from "./columns";
import { fetchTimeTableStructuredData } from "./data";
...
export default function InsertTimeTable() {
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRow: (row) => {
        skipAutoResetPageIndex();
        setTimeTableData((old) => old.filter((r) => r.id !== row.original.id));
      },
      deleteRows: (rows) => {
        skipAutoResetPageIndex();
        const ids = rows.map((r) => r.original.id);
        setTimeTableData((old) => old.filter((r) => !ids.includes(r.id)));
      },
    },
