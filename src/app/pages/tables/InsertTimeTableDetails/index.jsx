// src/app/pages/tables/InsertTimeTableDetails/index.jsx
...
import { generateTimeTableDetailColumns } from "./columns";
import { fetchTimeTableDetails } from "./data";
import { getUserAgentBrowser } from "utils/dom/getUserAgentBrowser";
import { useThemeContext } from "app/contexts/theme/context";
import InsertTimeTableDetailForm from "app/pages/forms/InsertTimeTableDetailForm";

const isSafari = getUserAgentBrowser() === "Safari";

export default function InsertTimeTableDetails() {
  ...
  const [columnVisibility, setColumnVisibility] = useLocalStorage("column-visibility-insert-timetable-details", {});
  const [columnPinning, setColumnPinning] = useLocalStorage("column-pinning-insert-timetable-details", {});
  ...
  const load = async () => {
    const { tableHeaders = [], data = [] } = await fetchTimeTableDetails(1);
    setTableData(data);
    setColumns(generateTimeTableDetailColumns(tableHeaders));
  };
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteRow: (row) => {
        ...
        setTableData((old) => old.filter((r) => r.id !== row.original.id));
      },
      deleteRows: (rows) => {
        skipAutoResetPageIndex();
        const ids = rows.map((r) => r.original.id);
        setTableData((old) => old.filter((r) => !ids.includes(r.id)));
      },
    },
  ...
  return (
    <>
      <div className="transition-content grid grid-cols-1 px-(--margin-x) py-4">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span>New Time Table Detail</span>
          </Button>
        </div>
      ...
      {/* Dialog for add form (update to correct form if needed) */}
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="relative w-full max-w-5xl overflow-y-auto rounded-lg bg-white p-6 shadow-lg dark:bg-dark-800">
            <div className="flex items-center justify-between pb-4">
              <h3 className="text-lg font-semibold text-gray-800 dark:text-dark-100">Add Time Table Detail</h3>
              <Button variant="outlined" size="icon-sm" onClick={() => setIsFormOpen(false)}>
                <XMarkIcon className="size-5" />
              </Button>
            </div>
            {/* TODO: Replace with InsertTimeTableDetailForm if needed */}
            <InsertTimeTableDetailForm onSuccess={() => { load(); setIsFormOpen(false); }} />
          </Dialog.Panel>
        </div>
      </Dialog>
    </>
