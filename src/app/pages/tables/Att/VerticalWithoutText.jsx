...
const AttendanceHeaderBox = ({
  ...
  return (
    <Box className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 rounded-lg shadow-sm bg-gradient-to-r from-primary-50/80 to-primary-100/60 dark:from-dark-800 dark:to-dark-700">
      {/* Date */}
      <div className="flex items-center gap-2 text-sm font-medium text-primary-950 dark:text-white">
        <CalendarDaysIcon className="size-5 shrink-0 text-primary-600" />
        <span>{date}</span>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm w-full sm:w-auto">
        ...
        <Listbox value={localSelectedCourse} onChange={setLocalSelectedCourse}>
          <div className="relative w-full sm:w-48">
            <Listbox.Button className="relative w-full cursor-pointer rounded-md border border-gray-300 bg-white px-2 py-1 pr-8 text-left text-sm font-medium text-gray-900 dark:bg-dark-200 dark:text-white">
              <span className="block truncate">
                {localSelectedCourse?.name ?? "Select Course"}
              </span>
              <span className="pointer-events-none absolute inset-y-0 right-2 flex items-center">
                <ChevronUpDownIcon className="h-4 w-4 text-gray-400" />
              </span>
            </Listbox.Button>
            <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-gray-200 bg-white py-1 text-sm shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:border-dark-500 dark:bg-dark-200 dark:text-white">
              {courseList.map((course) => (
                <Listbox.Option
                  key={course.id}
                  value={course}
                  className={({ active }) =>
                    `relative cursor-pointer select-none px-4 py-2 ${
                      active
                        ? "bg-primary-50 text-primary-900 dark:bg-dark-400"
      ...
      {/* Attendance Counts */}
      <div className="flex flex-col items-end text-sm space-y-1 sm:text-right">
        <div className="flex items-center gap-1 text-green-600">
          <ArrowDownCircleIcon className="size-4" />
          <span className="font-semibold">Checked-In: {checkedIn}</span>
        </div>
        <div className="flex items-center gap-1 text-red-500">
          <ArrowUpCircleIcon className="size-4" />
          <span className="font-semibold">Checked-Out: {checkedOut}</span>
        </div>
      </div>
    </Box>
  );
