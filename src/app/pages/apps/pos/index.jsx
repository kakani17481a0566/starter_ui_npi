...
export default function Pos() {
  ...
  const addToBasket = (course) => {
    setBasketItems((prev) => {
      ...
      if (existing) {
        ...
        toast.success(`${course.name} quantity increased`, {
          position: "top-center",
        });
        return updated;
  ...
  const handleIncrease = (uid) => {
    setBasketItems((prev) => prev.map((item) => item.uid === uid ?
      { ...item, count: item.count + 1 } : item));
  };
  const handleDecrease = (uid) => {
    setBasketItems((prev) => prev.map((item) => item.uid === uid && item.count > 1
      ? { ...item, count: item.count - 1 } : item));
  };
  return (
    <Page title="Point of Sales App">
      <Header /> <main className="main-content transition-content grid grid-cols-12 gap-4 px-(--margin-x) pb-6 pt-5 sm:gap-5 lg:gap-6">
        ...
        <div className="col-span-12 sm:col-span-6 lg:col-span-5 xl:col-span-4">
          <div className="sm:sticky sm:top-20 max-sm:fixed max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:z-50 max-sm:bg-white dark:max-sm:bg-dark-800">
            <Basket items={basketItems} onIncrease={handleIncrease} onDecrease={handleDecrease} />
          </div>
        </div>
      </main>
      <Sidebar />
