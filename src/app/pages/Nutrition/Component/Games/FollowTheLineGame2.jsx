...
// --- CONFIGURATION ---
// Path is defined by absolute coordinates relative to the container (at a 560x760 size)
// The path logic scales gracefully with the container, ensuring it stays centered.
const PATH_POINTS = [
  { x: 280, y: 120 }, // Start (Top Center)
  { x: 420, y: 250 }, // Right
  { x: 140, y: 380 }, // Left
  { x: 420, y: 510 }, // Right
  { x: 280, y: 640 }, // End (Center, above goal)
];

const DRAG_SIZE = 80;
const DRAG_HALF_SIZE = DRAG_SIZE / 2; // 40
const PATH_HALF_WIDTH = 36; // Tolerance distance from the center of the line
const PATH_TOLERANCE_SQUARED = PATH_HALF_WIDTH * PATH_HALF_WIDTH;
const FAIL_RESET_DELAY = 900;
// --- END CONFIGURATION ---

// Helper function to calculate the squared distance from a point p to a line segment defined by a and b.
function distToSegmentSquared(p, a, b) {
  const l2 = (b.x - a.x) ** 2 + (b.y - a.y) ** 2;
  if (l2 === 0) return (p.x - a.x) ** 2 + (p.y - a.y) ** 2; // a == b

  // Project point p onto the line segment (a, b)
  let t = ((p.x - a.x) * (b.x - a.x) + (p.y - a.y) * (b.y - a.y)) / l2;
  // Clamp t to [0, 1] to ensure we only measure distance to the segment itself
  t = Math.max(0, Math.min(1, t));

  const projectedX = a.x + t * (b.x - a.x);
  const projectedY = a.y + t * (b.y - a.y);

  return (p.x - projectedX) ** 2 + (p.y - projectedY) ** 2;
}

export default function FollowTheLineGame2() {
  ...
  function moveToClient(clientX, clientY) {
    ...
    // Calculate the scale factor relative to the defined 560px width
    const scaleFactor = cont.width / 560;

    // Use the defined size constants for calculation
    const dragW = DRAG_SIZE * scaleFactor;
    const dragH = DRAG_SIZE * scaleFactor;
    ...
    checkOnPath(clampedX, clampedY, scaleFactor);
  }
  ...
  // --- New Path Checking Logic ---
  function checkOnPath(x, y, scaleFactor) {
    if (locked) return;

    // Scale the tolerance based on the container size
    const scaledToleranceSq = PATH_TOLERANCE_SQUARED * scaleFactor * scaleFactor;

    let minDistanceSq = Infinity;
    const currentPos = { x, y };

    // 1. Check distance to all segments
    for (let i = 0; i < PATH_POINTS.length - 1; i++) {
        // Since points are defined for 560px width, scale them to the current width
        const a = { x: PATH_POINTS[i].x * scaleFactor, y: PATH_POINTS[i].y * scaleFactor };
        const b = { x: PATH_POINTS[i+1].x * scaleFactor, y: PATH_POINTS[i+1].y * scaleFactor };

        const distSq = distToSegmentSquared(currentPos, a, b);
        minDistanceSq = Math.min(minDistanceSq, distSq);
    }

    if (minDistanceSq > scaledToleranceSq) {
      triggerFail();
      return;
    }

    // 2. Check for success (close to the last point)
    const endPointRef = PATH_POINTS[PATH_POINTS.length - 1];
    const endPoint = { x: endPointRef.x * scaleFactor, y: endPointRef.y * scaleFactor };

    const distanceToEndSq = (x - endPoint.x) ** 2 + (y - endPoint.y) ** 2;

    // Check if the character is close enough to the end point to succeed
    if (distanceToEndSq < scaledToleranceSq * 2) {
      triggerSuccess();
    }
  }
  // --- End New Path Checking Logic ---
  ...
  function resetDraggableToStart() {
    // Start position is always the first point in the path definition
    setPos(PATH_POINTS[0]);
    setMessage("");
    setLocked(false);
  }
  ...
  // Generates the SVG 'd' path string from the points array
  function getSvgPathD(points) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-100" style={{ fontFamily: "Poppins, Inter, system-ui, sans-serif" }}>
      ...
      <div
        ref={containerRef}
        // Set dimensions and allow scaling for the absolute path coordinates
      ...
      >
        ...
        <div className="absolute left-4 right-4 top-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="text-xl font-extrabold text-slate-800 drop-shadow-sm">Follow the Zig-Zag</div>
          </div>
          <button
        ...
        <div className="absolute top-16 left-6 right-6 text-center text-sm text-slate-700 z-10">
          <div className="bg-white/70 inline-block px-3 py-1 rounded-full shadow">Drag the happy face along the path — don’t wobble off!</div>
        </div>

        {/* playful SVG zig-zag path */}
        <svg
            className="absolute inset-0 w-full h-full"
            viewBox={`0 0 560 760`} // Match the largest container size for coordinates
            preserveAspectRatio="xMidYMid slice" // Ensures the path scales correctly inside the container
            aria-hidden="true"
        >
            {/* Glowing line (inner core) */}
            <path
                d={getSvgPathD(PATH_POINTS)}
                fill="none"
                stroke="url(#pathGradient)"
                strokeWidth={PATH_HALF_WIDTH * 2.5} // Slightly wider than tolerance
                strokeLinecap="round"
            ...
            />
            {/* Dotted line (outer appearance) */}
            <path
                d={getSvgPathD(PATH_POINTS)}
                fill="none"
                stroke="#fff"
                strokeWidth={PATH_HALF_WIDTH * 0.8}
                strokeDasharray="0.1, 15" // Creates a sparse dotted effect
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            <defs>
                <linearGradient id="pathGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                    ...
                    <stop offset="100%" style={{stopColor:"#6bd0ff", stopOpacity:1}} />
                </linearGradient>
            </defs>

        </svg>

        {/* goal (friend) - positioned based on the last point */}
        <div
            className="absolute text-center z-10"
            style={{
                left: `${lastPoint.x}px`,
                top: `${lastPoint.y}px`,
                transform: "translate(-50%, -50%)", // Center on the last path point
            }}
        >
          ...
          <div className="mt-2 text-sm text-slate-700 font-medium">Goal!</div>
        </div>
